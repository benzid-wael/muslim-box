...
const Adhan = (props: StateProps): React$Node => {
  ...
  const [audio] = useState<?Audio>(new Audio(adhan.sound));
  ...
  return (
    <Main onClick={togglePlay} playing={state.play}>
      <Wrapper>
        <Title>{i18n.t("Now is the time for {{prayer}} prayer", { prayer })}</Title>
        {state.ended ? <DuaaAfterAdhan direction={props.direction} /> : <DuaaDuringAdhan direction={props.direction} />}
      </Wrapper>
    </Main>
  );
};
